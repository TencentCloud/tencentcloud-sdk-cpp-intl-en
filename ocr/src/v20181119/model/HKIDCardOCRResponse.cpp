...
CoreInternalOutcome HKIDCardOCRResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("HKIDVersion") && !rsp["HKIDVersion"].IsNull())
    {
        if (!rsp["HKIDVersion"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `HKIDVersion` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_hKIDVersion = string(rsp["HKIDVersion"].GetString());
...
string HKIDCardOCRResponse::GetHKIDVersion() const
{
    return m_hKIDVersion;
}

bool HKIDCardOCRResponse::HKIDVersionHasBeenSet() const
{
