...
CoreInternalOutcome InputStatistics::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Pipeline0") && !value["Pipeline0"].IsNull())
    {
        if (!value["Pipeline0"].IsArray())
            return CoreInternalOutcome(Error("response `InputStatistics.Pipeline0` is not array type"));
    ...
    if (value.HasMember("Pipeline1") && !value["Pipeline1"].IsNull())
    {
        if (!value["Pipeline1"].IsArray())
            return CoreInternalOutcome(Error("response `InputStatistics.Pipeline1` is not array type"));
...
void InputStatistics::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
