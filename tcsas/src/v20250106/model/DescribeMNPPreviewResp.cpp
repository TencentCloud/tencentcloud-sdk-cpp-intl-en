...
CoreInternalOutcome DescribeMNPPreviewResp::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("QRCodeContent") && !value["QRCodeContent"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `DescribeMNPPreviewResp.QRCodeContent` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeMNPPreviewResp::GetQRCodeContent() const
{
    return m_qRCodeContent;
}

void DescribeMNPPreviewResp::SetQRCodeContent(const string& _qRCodeContent)
{
    m_qRCodeContent = _qRCodeContent;
    m_qRCodeContentHasBeenSet = true;
}

bool DescribeMNPPreviewResp::QRCodeContentHasBeenSet() const
{
