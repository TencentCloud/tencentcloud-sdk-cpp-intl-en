...
#include <tencentcloud/tcsas/v20250106/model/UploadFileTempSecret.h>
...
UploadFileTempSecret::UploadFileTempSecret() :
    ...
    m_accelerateEnableHasBeenSet(false)
{
}
...
CoreInternalOutcome UploadFileTempSecret::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("AccelerateEnable") && !value["AccelerateEnable"].IsNull())
    {
        if (!value["AccelerateEnable"].IsInt64())
        {
            return CoreInternalOutcome(Core::Error("response `UploadFileTempSecret.AccelerateEnable` IsInt64=false incorrectly").SetRequestId(requestId));
        }
        m_accelerateEnable = value["AccelerateEnable"].GetInt64();
        m_accelerateEnableHasBeenSet = true;
    }
...
void UploadFileTempSecret::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_accelerateEnableHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AccelerateEnable";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_accelerateEnable, allocator);
    }
...
int64_t UploadFileTempSecret::GetAccelerateEnable() const
{
    return m_accelerateEnable;
}

void UploadFileTempSecret::SetAccelerateEnable(const int64_t& _accelerateEnable)
{
    m_accelerateEnable = _accelerateEnable;
    m_accelerateEnableHasBeenSet = true;
}

bool UploadFileTempSecret::AccelerateEnableHasBeenSet() const
{
    return m_accelerateEnableHasBeenSet;
}
