...
{
    using Req = const DescribeStreamLinkSecurityGroupRequest&;
    using Resp = DescribeStreamLinkSecurityGroupResponse;

    DoRequestAsync<Req, Resp>(
        "DescribeStreamLinkSecurityGroup", request, {{{"Content-Type", "application/json"}}},
        [this, context, handler](Req req, Outcome<Core::Error, Resp> resp)
        {
            handler(this, req, std::move(resp), context);
        });
...
{
    const auto prom = std::make_shared<std::promise<DescribeStreamLinkSecurityGroupOutcome>>();
    DescribeStreamLinkSecurityGroupAsync(
    request,
    [prom](
        const MpsClient*,
