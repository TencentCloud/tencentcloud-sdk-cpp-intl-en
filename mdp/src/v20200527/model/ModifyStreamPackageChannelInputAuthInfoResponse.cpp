...
CoreInternalOutcome ModifyStreamPackageChannelInputAuthInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AuthInfo") && !rsp["AuthInfo"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AuthInfo` is not object type").SetRequestId(requestId));
        }
...
string ModifyStreamPackageChannelInputAuthInfoResponse::ToJsonString() const
{
...
InputAuthInfo ModifyStreamPackageChannelInputAuthInfoResponse::GetAuthInfo() const
{
    return m_authInfo;
}

bool ModifyStreamPackageChannelInputAuthInfoResponse::AuthInfoHasBeenSet() const
{
