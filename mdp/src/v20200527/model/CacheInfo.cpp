...
CoreInternalOutcome CacheInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Info") && !value["Info"].IsNull())
    {
        if (!value["Info"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CacheInfo.Info` is not array type"));
...
vector<CacheInfoInfo> CacheInfo::GetInfo() const
{
    return m_info;
}

void CacheInfo::SetInfo(const vector<CacheInfoInfo>& _info)
{
    m_info = _info;
    m_infoHasBeenSet = true;
}

bool CacheInfo::InfoHasBeenSet() const
{
