/*
 * Copyright (c) 2017-2019 THL A29 Limited, a Tencent company. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include <tencentcloud/mdp/v20200527/model/DeleteMediaPackageChannelsResponse.h>
#include <tencentcloud/core/utils/rapidjson/document.h>
#include <tencentcloud/core/utils/rapidjson/writer.h>
#include <tencentcloud/core/utils/rapidjson/stringbuffer.h>

using TencentCloud::CoreInternalOutcome;
using namespace TencentCloud::Mdp::V20200527::Model;
using namespace rapidjson;
using namespace std;

DeleteMediaPackageChannelsResponse::DeleteMediaPackageChannelsResponse() :
    m_successInfosHasBeenSet(false),
    m_failInfosHasBeenSet(false)
{
}

CoreInternalOutcome DeleteMediaPackageChannelsResponse::Deserialize(const string &payload)
{
    Document d;
    d.Parse(payload.c_str());
    if (d.HasParseError() || !d.IsObject())
    {
        return CoreInternalOutcome(Error("response not json format"));
    }
    if (!d.HasMember("Response") || !d["Response"].IsObject())
    {
        return CoreInternalOutcome(Error("response `Response` is null or not object"));
    }
    Value &rsp = d["Response"];
    if (!rsp.HasMember("RequestId") || !rsp["RequestId"].IsString())
    {
        return CoreInternalOutcome(Error("response `Response.RequestId` is null or not string"));
    }
    string requestId(rsp["RequestId"].GetString());
    SetRequestId(requestId);

    if (rsp.HasMember("Error"))
    {
        if (!rsp["Error"].IsObject() ||
            !rsp["Error"].HasMember("Code") || !rsp["Error"]["Code"].IsString() ||
            !rsp["Error"].HasMember("Message") || !rsp["Error"]["Message"].IsString())
        {
            return CoreInternalOutcome(Error("response `Response.Error` format error").SetRequestId(requestId));
        }
        string errorCode(rsp["Error"]["Code"].GetString());
        string errorMsg(rsp["Error"]["Message"].GetString());
        return CoreInternalOutcome(Error(errorCode, errorMsg).SetRequestId(requestId));
    }


    if (rsp.HasMember("SuccessInfos") && !rsp["SuccessInfos"].IsNull())
    {
        if (!rsp["SuccessInfos"].IsArray())
            return CoreInternalOutcome(Error("response `SuccessInfos` is not array type"));

        const Value &tmpValue = rsp["SuccessInfos"];
        for (Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ChannelInfo item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
            if (!outcome.IsSuccess())
            {
                outcome.GetError().SetRequestId(requestId);
                return outcome;
            }
            m_successInfos.push_back(item);
        }
        m_successInfosHasBeenSet = true;
    }

    if (rsp.HasMember("FailInfos") && !rsp["FailInfos"].IsNull())
    {
        if (!rsp["FailInfos"].IsArray())
            return CoreInternalOutcome(Error("response `FailInfos` is not array type"));

        const Value &tmpValue = rsp["FailInfos"];
        for (Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ChannelInfo item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
            if (!outcome.IsSuccess())
            {
                outcome.GetError().SetRequestId(requestId);
                return outcome;
            }
            m_failInfos.push_back(item);
        }
        m_failInfosHasBeenSet = true;
    }


    return CoreInternalOutcome(true);
}


vector<ChannelInfo> DeleteMediaPackageChannelsResponse::GetSuccessInfos() const
{
    return m_successInfos;
}

bool DeleteMediaPackageChannelsResponse::SuccessInfosHasBeenSet() const
{
    return m_successInfosHasBeenSet;
}

vector<ChannelInfo> DeleteMediaPackageChannelsResponse::GetFailInfos() const
{
    return m_failInfos;
}

bool DeleteMediaPackageChannelsResponse::FailInfosHasBeenSet() const
{
    return m_failInfosHasBeenSet;
}


