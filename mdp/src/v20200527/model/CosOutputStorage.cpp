...
CoreInternalOutcome CosOutputStorage::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("OutputDir") && !value["OutputDir"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CosOutputStorage.OutputDir` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string CosOutputStorage::GetOutputDir() const
{
    return m_outputDir;
}

void CosOutputStorage::SetOutputDir(const string& _outputDir)
{
    m_outputDir = _outputDir;
    m_outputDirHasBeenSet = true;
}

bool CosOutputStorage::OutputDirHasBeenSet() const
{
