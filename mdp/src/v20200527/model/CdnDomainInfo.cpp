...
CoreInternalOutcome CdnDomainInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("TotalSize") && !value["TotalSize"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CdnDomainInfo.TotalSize` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Records") && !value["Records"].IsNull())
    {
        if (!value["Records"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CdnDomainInfo.Records` is not array type"));

        const rapidjson::Value &tmpValue = value["Records"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            DomainRecordInfo item;
...
int64_t CdnDomainInfo::GetTotalSize() const
{
    return m_totalSize;
}

void CdnDomainInfo::SetTotalSize(const int64_t& _totalSize)
{
    m_totalSize = _totalSize;
    m_totalSizeHasBeenSet = true;
}

bool CdnDomainInfo::TotalSizeHasBeenSet() const
{
    return m_totalSizeHasBeenSet;
}

vector<DomainRecordInfo> CdnDomainInfo::GetRecords() const
{
    return m_records;
}

void CdnDomainInfo::SetRecords(const vector<DomainRecordInfo>& _records)
{
    m_records = _records;
    m_recordsHasBeenSet = true;
}

bool CdnDomainInfo::RecordsHasBeenSet() const
{
