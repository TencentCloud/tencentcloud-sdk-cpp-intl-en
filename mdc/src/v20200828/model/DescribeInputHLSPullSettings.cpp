...
CoreInternalOutcome DescribeInputHLSPullSettings::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("SourceAddresses") && !value["SourceAddresses"].IsNull())
    {
        if (!value["SourceAddresses"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DescribeInputHLSPullSettings.SourceAddresses` is not array type"));

        const rapidjson::Value &tmpValue = value["SourceAddresses"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            DescribeHLSPullSourceAddress item;
...
void DescribeInputHLSPullSettings::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<DescribeHLSPullSourceAddress> DescribeInputHLSPullSettings::GetSourceAddresses() const
{
    return m_sourceAddresses;
}

void DescribeInputHLSPullSettings::SetSourceAddresses(const vector<DescribeHLSPullSourceAddress>& _sourceAddresses)
{
    m_sourceAddresses = _sourceAddresses;
    m_sourceAddressesHasBeenSet = true;
}

bool DescribeInputHLSPullSettings::SourceAddressesHasBeenSet() const
{
