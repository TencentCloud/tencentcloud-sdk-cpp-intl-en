...
CoreInternalOutcome DescribeInputRTMPPullSettings::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("SourceAddresses") && !value["SourceAddresses"].IsNull())
    {
        if (!value["SourceAddresses"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DescribeInputRTMPPullSettings.SourceAddresses` is not array type"));

        const rapidjson::Value &tmpValue = value["SourceAddresses"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            DescribeRTMPPullSourceAddress item;
...
void DescribeInputRTMPPullSettings::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<DescribeRTMPPullSourceAddress> DescribeInputRTMPPullSettings::GetSourceAddresses() const
{
    return m_sourceAddresses;
}

void DescribeInputRTMPPullSettings::SetSourceAddresses(const vector<DescribeRTMPPullSourceAddress>& _sourceAddresses)
{
    m_sourceAddresses = _sourceAddresses;
    m_sourceAddressesHasBeenSet = true;
}

bool DescribeInputRTMPPullSettings::SourceAddressesHasBeenSet() const
{
