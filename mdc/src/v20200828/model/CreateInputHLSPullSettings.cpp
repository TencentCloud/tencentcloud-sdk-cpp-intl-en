...
CoreInternalOutcome CreateInputHLSPullSettings::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("SourceAddresses") && !value["SourceAddresses"].IsNull())
    {
        if (!value["SourceAddresses"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CreateInputHLSPullSettings.SourceAddresses` is not array type"));

        const rapidjson::Value &tmpValue = value["SourceAddresses"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            HLSPullSourceAddress item;
...
void CreateInputHLSPullSettings::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<HLSPullSourceAddress> CreateInputHLSPullSettings::GetSourceAddresses() const
{
    return m_sourceAddresses;
}

void CreateInputHLSPullSettings::SetSourceAddresses(const vector<HLSPullSourceAddress>& _sourceAddresses)
{
    m_sourceAddresses = _sourceAddresses;
    m_sourceAddressesHasBeenSet = true;
}

bool CreateInputHLSPullSettings::SourceAddressesHasBeenSet() const
{
