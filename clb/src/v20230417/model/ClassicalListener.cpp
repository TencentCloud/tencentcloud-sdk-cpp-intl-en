...
CoreInternalOutcome ClassicalListener::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("AbnormalNum") && !value["AbnormalNum"].IsNull())
    {
        if (!value["AbnormalNum"].IsInt64())
        {
            return CoreInternalOutcome(Core::Error("response `ClassicalListener.AbnormalNum` IsInt64=false incorrectly").SetRequestId(requestId));
        }
        m_abnormalNum = value["AbnormalNum"].GetInt64();
        m_abnormalNumHasBeenSet = true;
    }
...
void ClassicalListener::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_abnormalNumHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AbnormalNum";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_abnormalNum, allocator);
    }
...
int64_t ClassicalListener::GetAbnormalNum() const
{
    return m_abnormalNum;
}

void ClassicalListener::SetAbnormalNum(const int64_t& _abnormalNum)
{
    m_abnormalNum = _abnormalNum;
    m_abnormalNumHasBeenSet = true;
}

bool ClassicalListener::AbnormalNumHasBeenSet() const
{
    return m_abnormalNumHasBeenSet;
}
