...
CoreInternalOutcome BindDetailItem::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ConfigId") && !value["ConfigId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `BindDetailItem.ConfigId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string BindDetailItem::GetConfigId() const
{
    return m_configId;
}

void BindDetailItem::SetConfigId(const string& _configId)
{
    m_configId = _configId;
    m_configIdHasBeenSet = true;
}

bool BindDetailItem::ConfigIdHasBeenSet() const
{
