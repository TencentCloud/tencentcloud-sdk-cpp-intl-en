...
#include <tencentcloud/tcmpp/v20240801/model/DescribeUserListData.h>
...
CoreInternalOutcome DescribeUserListData::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["Teams"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `DescribeUserListData.Teams` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_teams = string(value["Teams"].GetString());
...
void DescribeUserListData::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_teamsHasBeenSet)
    {
        ...
        string key = "Teams";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_teams.c_str(), allocator).Move(), allocator);
    }
...
string DescribeUserListData::GetTeams() const
{
    return m_teams;
}

void DescribeUserListData::SetTeams(const string& _teams)
{
    m_teams = _teams;
    m_teamsHasBeenSet = true;
}

bool DescribeUserListData::TeamsHasBeenSet() const
{
