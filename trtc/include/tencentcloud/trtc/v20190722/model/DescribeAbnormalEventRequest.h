/*
 * Copyright (c) 2017-2019 THL A29 Limited, a Tencent company. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef TENCENTCLOUD_TRTC_V20190722_MODEL_DESCRIBEABNORMALEVENTREQUEST_H_
#define TENCENTCLOUD_TRTC_V20190722_MODEL_DESCRIBEABNORMALEVENTREQUEST_H_

#include <string>
#include <vector>
#include <map>
#include <tencentcloud/core/AbstractModel.h>


namespace TencentCloud
{
    namespace Trtc
    {
        namespace V20190722
        {
            namespace Model
            {
                /**
                * DescribeAbnormalEvent request structure.
                */
                class DescribeAbnormalEventRequest : public AbstractModel
                {
                public:
                    DescribeAbnormalEventRequest();
                    ~DescribeAbnormalEventRequest() = default;
                    std::string ToJsonString() const;


                    /**
                     * 获取User `SDKAppID`, which can be used to query 20 exceptional experience events (in one or more rooms)
                     * @return SdkAppId User `SDKAppID`, which can be used to query 20 exceptional experience events (in one or more rooms)
                     */
                    std::string GetSdkAppId() const;

                    /**
                     * 设置User `SDKAppID`, which can be used to query 20 exceptional experience events (in one or more rooms)
                     * @param SdkAppId User `SDKAppID`, which can be used to query 20 exceptional experience events (in one or more rooms)
                     */
                    void SetSdkAppId(const std::string& _sdkAppId);

                    /**
                     * 判断参数 SdkAppId 是否已赋值
                     * @return SdkAppId 是否已赋值
                     */
                    bool SdkAppIdHasBeenSet() const;

                    /**
                     * 获取Query start time (s) in the format of Unix timestamp, e.g., 1592448600
                     * @return StartTime Query start time (s) in the format of Unix timestamp, e.g., 1592448600
                     */
                    uint64_t GetStartTime() const;

                    /**
                     * 设置Query start time (s) in the format of Unix timestamp, e.g., 1592448600
                     * @param StartTime Query start time (s) in the format of Unix timestamp, e.g., 1592448600
                     */
                    void SetStartTime(const uint64_t& _startTime);

                    /**
                     * 判断参数 StartTime 是否已赋值
                     * @return StartTime 是否已赋值
                     */
                    bool StartTimeHasBeenSet() const;

                    /**
                     * 获取Query end time (s) in the format of Unix timestamp, e.g., 1592449080
                     * @return EndTime Query end time (s) in the format of Unix timestamp, e.g., 1592449080
                     */
                    uint64_t GetEndTime() const;

                    /**
                     * 设置Query end time (s) in the format of Unix timestamp, e.g., 1592449080
                     * @param EndTime Query end time (s) in the format of Unix timestamp, e.g., 1592449080
                     */
                    void SetEndTime(const uint64_t& _endTime);

                    /**
                     * 判断参数 EndTime 是否已赋值
                     * @return EndTime 是否已赋值
                     */
                    bool EndTimeHasBeenSet() const;

                    /**
                     * 获取Room ID, which can be used to query up to 20 exceptional experience events in a specific room
                     * @return RoomId Room ID, which can be used to query up to 20 exceptional experience events in a specific room
                     */
                    std::string GetRoomId() const;

                    /**
                     * 设置Room ID, which can be used to query up to 20 exceptional experience events in a specific room
                     * @param RoomId Room ID, which can be used to query up to 20 exceptional experience events in a specific room
                     */
                    void SetRoomId(const std::string& _roomId);

                    /**
                     * 判断参数 RoomId 是否已赋值
                     * @return RoomId 是否已赋值
                     */
                    bool RoomIdHasBeenSet() const;

                private:

                    /**
                     * User `SDKAppID`, which can be used to query 20 exceptional experience events (in one or more rooms)
                     */
                    std::string m_sdkAppId;
                    bool m_sdkAppIdHasBeenSet;

                    /**
                     * Query start time (s) in the format of Unix timestamp, e.g., 1592448600
                     */
                    uint64_t m_startTime;
                    bool m_startTimeHasBeenSet;

                    /**
                     * Query end time (s) in the format of Unix timestamp, e.g., 1592449080
                     */
                    uint64_t m_endTime;
                    bool m_endTimeHasBeenSet;

                    /**
                     * Room ID, which can be used to query up to 20 exceptional experience events in a specific room
                     */
                    std::string m_roomId;
                    bool m_roomIdHasBeenSet;

                };
            }
        }
    }
}

#endif // !TENCENTCLOUD_TRTC_V20190722_MODEL_DESCRIBEABNORMALEVENTREQUEST_H_
