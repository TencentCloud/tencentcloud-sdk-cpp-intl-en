...
CoreInternalOutcome MaxAgeRule::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("MaxAgeTime") && !value["MaxAgeTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `MaxAgeRule.MaxAgeTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t MaxAgeRule::GetMaxAgeTime() const
{
    return m_maxAgeTime;
}

void MaxAgeRule::SetMaxAgeTime(const uint64_t& _maxAgeTime)
{
