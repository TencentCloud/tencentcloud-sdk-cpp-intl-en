...
CoreInternalOutcome AuthenticationTypeB::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ExpireTime") && !value["ExpireTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AuthenticationTypeB.ExpireTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t AuthenticationTypeB::GetExpireTime() const
{
    return m_expireTime;
}

void AuthenticationTypeB::SetExpireTime(const uint64_t& _expireTime)
{
