...
CoreInternalOutcome ForceRedirect::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["RedirectStatusCode"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `ForceRedirect.RedirectStatusCode` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_redirectStatusCode = value["RedirectStatusCode"].GetUint64();
...
uint64_t ForceRedirect::GetRedirectStatusCode() const
{
    return m_redirectStatusCode;
}

void ForceRedirect::SetRedirectStatusCode(const uint64_t& _redirectStatusCode)
{
