...
CoreInternalOutcome AuthenticationTypeA::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ExpireTime") && !value["ExpireTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AuthenticationTypeA.ExpireTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t AuthenticationTypeA::GetExpireTime() const
{
    return m_expireTime;
}

void AuthenticationTypeA::SetExpireTime(const uint64_t& _expireTime)
{
