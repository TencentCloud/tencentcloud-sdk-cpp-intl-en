...
CoreInternalOutcome AuthenticationTypeD::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ExpireTime") && !value["ExpireTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AuthenticationTypeD.ExpireTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t AuthenticationTypeD::GetExpireTime() const
{
    return m_expireTime;
}

void AuthenticationTypeD::SetExpireTime(const uint64_t& _expireTime)
{
