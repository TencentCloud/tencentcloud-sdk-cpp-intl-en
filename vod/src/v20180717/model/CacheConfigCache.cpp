...
CoreInternalOutcome CacheConfigCache::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("CacheTime") && !value["CacheTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CacheConfigCache.CacheTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t CacheConfigCache::GetCacheTime() const
{
    return m_cacheTime;
}

void CacheConfigCache::SetCacheTime(const uint64_t& _cacheTime)
{
