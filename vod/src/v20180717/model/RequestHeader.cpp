...
CoreInternalOutcome RequestHeader::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Switch") && !value["Switch"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `RequestHeader.Switch` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("HeaderRules") && !value["HeaderRules"].IsNull())
    {
        if (!value["HeaderRules"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RequestHeader.HeaderRules` is not array type"));
...
void RequestHeader::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
