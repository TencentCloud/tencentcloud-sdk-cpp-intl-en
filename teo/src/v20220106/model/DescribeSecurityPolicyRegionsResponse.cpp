...
CoreInternalOutcome DescribeSecurityPolicyRegionsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("GeoIp") && !rsp["GeoIp"].IsNull())
    {
        if (!rsp["GeoIp"].IsArray())
            return CoreInternalOutcome(Core::Error("response `GeoIp` is not array type"));

        const rapidjson::Value &tmpValue = rsp["GeoIp"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ...
            m_geoIp.push_back(item);
        }
        m_geoIpHasBeenSet = true;
    }
...
string DescribeSecurityPolicyRegionsResponse::ToJsonString() const
{
    ...
    if (m_geoIpHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_geoIp.begin(); itr != m_geoIp.end(); ++itr, ++i)
        {
...
vector<GeoIp> DescribeSecurityPolicyRegionsResponse::GetGeoIp() const
{
    return m_geoIp;
}

bool DescribeSecurityPolicyRegionsResponse::GeoIpHasBeenSet() const
{
    return m_geoIpHasBeenSet;
}
