...
CoreInternalOutcome TimingTypeValue::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("DetailData") && !value["DetailData"].IsNull())
    {
        if (!value["DetailData"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TimingTypeValue.DetailData` is not array type"));

        const rapidjson::Value &tmpValue = value["DetailData"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            m_detailData.push_back((*itr).GetInt64());
        }
...
void TimingTypeValue::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_detailDataHasBeenSet)
    {
        ...
        for (auto itr = m_detailData.begin(); itr != m_detailData.end(); ++itr)
        {
...
vector<int64_t> TimingTypeValue::GetDetailData() const
{
    return m_detailData;
}

void TimingTypeValue::SetDetailData(const vector<int64_t>& _detailData)
{
    m_detailData = _detailData;
    m_detailDataHasBeenSet = true;
}

bool TimingTypeValue::DetailDataHasBeenSet() const
{
