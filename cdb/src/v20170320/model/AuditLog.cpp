...
CoreInternalOutcome AuditLog::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["IoWaitTime"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `AuditLog.IoWaitTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_ioWaitTime = value["IoWaitTime"].GetUint64();
        m_ioWaitTimeHasBeenSet = true;
    }

    if (value.HasMember("LockWaitTime") && !value["LockWaitTime"].IsNull())
    {
        if (!value["LockWaitTime"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `AuditLog.LockWaitTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_lockWaitTime = value["LockWaitTime"].GetUint64();
        m_lockWaitTimeHasBeenSet = true;
    }

    if (value.HasMember("NsTime") && !value["NsTime"].IsNull())
    {
        if (!value["NsTime"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `AuditLog.NsTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_nsTime = value["NsTime"].GetUint64();
        m_nsTimeHasBeenSet = true;
    }

    if (value.HasMember("TrxLivingTime") && !value["TrxLivingTime"].IsNull())
    {
        if (!value["TrxLivingTime"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `AuditLog.TrxLivingTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_trxLivingTime = value["TrxLivingTime"].GetUint64();
...
uint64_t AuditLog::GetIoWaitTime() const
{
    return m_ioWaitTime;
}

void AuditLog::SetIoWaitTime(const uint64_t& _ioWaitTime)
{
...
uint64_t AuditLog::GetLockWaitTime() const
{
    return m_lockWaitTime;
}

void AuditLog::SetLockWaitTime(const uint64_t& _lockWaitTime)
{
...
uint64_t AuditLog::GetNsTime() const
{
    return m_nsTime;
}

void AuditLog::SetNsTime(const uint64_t& _nsTime)
{
    m_nsTime = _nsTime;
    m_nsTimeHasBeenSet = true;
}
...
uint64_t AuditLog::GetTrxLivingTime() const
{
    return m_trxLivingTime;
}

void AuditLog::SetTrxLivingTime(const uint64_t& _trxLivingTime)
{
