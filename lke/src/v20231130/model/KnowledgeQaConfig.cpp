...
CoreInternalOutcome KnowledgeQaConfig::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("EnableAudit") && !value["EnableAudit"].IsNull())
    {
        if (!value["EnableAudit"].IsBool())
        {
            return CoreInternalOutcome(Core::Error("response `KnowledgeQaConfig.EnableAudit` IsBool=false incorrectly").SetRequestId(requestId));
        }
        m_enableAudit = value["EnableAudit"].GetBool();
...
void KnowledgeQaConfig::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_enableAuditHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_enableAudit, allocator);
    }
...
bool KnowledgeQaConfig::GetEnableAudit() const
{
    return m_enableAudit;
}

void KnowledgeQaConfig::SetEnableAudit(const bool& _enableAudit)
{
    m_enableAudit = _enableAudit;
    m_enableAuditHasBeenSet = true;
}

bool KnowledgeQaConfig::EnableAuditHasBeenSet() const
{
